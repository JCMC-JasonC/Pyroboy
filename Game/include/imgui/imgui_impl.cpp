...
namespace internal
{
	...
	std::shared_ptr<Texture> g_imguiFontTex;
...
void internal::StartUI(int windowWidth, int windowHeight)
{
...
void internal::imguiInit()
{
	...
	io.RenderDrawListsFn = internal::imguiDraw;

	g_imguiFontTex = std::make_shared<Texture>();
	...
	io.Fonts->TexID = static_cast<void*>(g_imguiFontTex.get());
...
void internal::InitImGUI()
{
	internal::imguiInit();
}
void internal::imguiDraw(ImDrawData* draw_data)
{
	ImGuiIO& io = ImGui::GetIO();
	...
	for (int n = 0; n < draw_data->CmdListsCount; n++)
	{
		...
		for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
		{
			...
			{
				glBindTexture(GL_TEXTURE_2D, (GLuint)((Texture*)pcmd->TextureId)->textObj);
